...
if st.session_state["page"] == "visit":
    ...
    def clear_form():
        if agent_list:
            ...
            st.session_state.agent_select = first_agent
            areas = customers[customers["Agent Name"] == first_agent]["Area"].dropna()
            areas = areas[areas != ""].unique().tolist()
            st.session_state[f"area_select_{first_agent}"] = areas[0] if areas else ""
            provinces = customers[
                (customers["Agent Name"] == first_agent) &
                (customers["Area"] == (areas[0] if areas else ""))
            ]["Province"].dropna()
            provinces = provinces[provinces != ""].unique().tolist()
            if len(provinces) == 1:
                st.session_state[f"province_display_{first_agent}_{areas[0] if areas else ''}"] = provinces[0]
            else:
                st.session_state[f"province_select_{first_agent}_{areas[0] if areas else ''}"] = provinces[0] if provinces else ""
